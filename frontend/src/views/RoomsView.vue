...
async function deleteRoom(id: string) {
  ...
  try {
    ...
    async function resetUsedQuestions(id: string) {
      if (!confirm('Reset used questions for this room?')) return
      try {
        await request(`/rooms/${id}/questions/reset_used`, 'POST', null, auth.token!)
        alert('Used questions reset!')
      } catch (e: any) {
        alert(e.message)
      }
    }
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="room in rooms" :key="room.id"
            ...
            <div class="flex w-full mt-2 gap-2">
               <button @click="resetUsedQuestions(room.id)"
                class="py-2 px-3 bg-yellow-600 hover:bg-yellow-500 text-slate-300 hover:text-white rounded-xl transition-all border border-white/10 flex items-center justify-center group/btn" title="Reset Used Questions">
