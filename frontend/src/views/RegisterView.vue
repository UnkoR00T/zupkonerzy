...
async function handleRegister() {
  ...
  try {
    ...
    auth.setAuth(data.token, { username: username.value, email: email.value })
    router.push('/rooms')
  } catch (e: any) {
  ...
  <div class="min-h-screen flex items-center justify-center p-6">
    <div class="glass-card w-full max-w-md p-8 space-y-8 animate-in fade-in zoom-in duration-500">
      <div class="text-center">
        <h1 class="text-4xl font-bold text-white mb-2">Join Us</h1>
        <p class="text-slate-400">Create an account to start creating quizzes</p>
      </div>
      ...
      <p class="text-center text-slate-400">
        ...
        <router-link to="/login" class="text-brand hover:text-brand-light font-medium transition-colors">Login</router-link>
      </p>
    </div>
  </div>
</template>
